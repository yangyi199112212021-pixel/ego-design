(function () {
  ...
  function responsiveImageMarkup(src, alt, className = "", sizes = "(max-width: 760px) 100vw, 33vw") {
    ...
    const image = `<img${classAttr} src="${escapeHtml(src)}" data-full-src="${escapeHtml(src)}" alt="${escapeHtml(alt)}" loading="lazy" decoding="async">`;
    if (!mobileSrc) return image;
    return `<picture><source media="(max-width: 760px)" srcset="${escapeHtml(mobileSrc)}">${image}</picture>`;
  }
