(function () {
  ...
  function setupMobileProjectVideos() {
    const cards = document.querySelectorAll(".project-card");
    const shouldAutoplay = window.matchMedia("(hover: none), (pointer: coarse), (max-width: 760px)").matches;
    if (!cards.length || !shouldAutoplay) return;
    ...
    function playVideo(card) {
      ...
      video.preload = "metadata";
      video.play()
        .then(() => card.classList.add("is-video-playing"))
        .catch(() => card.classList.remove("is-video-playing"));
    }

    if (!("IntersectionObserver" in window)) {
      cards.forEach(playVideo);
      return;
    }
